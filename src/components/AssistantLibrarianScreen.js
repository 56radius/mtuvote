...
function AssistantLibrarianScreen() {
  ...
  const proCandidates = candidates.filter((candidate) => candidate.post === 8);

  const handleVote = (candidateId) => {
    if (candidateId && token) {
      axios
        .post(
          `https://nacos-vote.onrender.com/voters/vote/8/${candidateId}`, // Replace with your actual backend URL
          null,
          {
            headers: {
...
export default AssistantLibrarianScreen;
