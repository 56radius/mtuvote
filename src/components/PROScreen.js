...
function PROScreen() {
  ...
  const proCandidates = candidates.filter((candidate) => candidate.post === 6);

  const handleVote = (candidateId) => {
    if (candidateId && token) {
      axios
        .post(
          `https://nacos-vote.onrender.com/voters/vote/6/${candidateId}`, // Replace with your actual backend URL
          null,
          {
            headers: {
        ...
        <div className="pagetitle">
          <h1>
            <span style={{ color: "green" }}>
              Pick your Public Relations Officer (PRO)
            </span>
          </h1>
        </div>
